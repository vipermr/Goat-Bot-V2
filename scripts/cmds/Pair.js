...
module.exports = {
  config: {
    ...
    longDescription: {
      en: "Know your destiny and who you'll pair with",
    },
    category: "love",
    guide: {
  ...
  onStart: async function ({ api, event, usersData }) {
    const pathImg = __dirname + "/assets/pair_bg.png";
    const pathAvt1 = __dirname + "/assets/pair_avt1.png";
    const pathAvt2 = __dirname + "/assets/pair_avt2.png";
    const backgroundUrl = "https://raw.githubusercontent.com/alkama844/res/refs/heads/main/image/pair.jpg";
    ...
    if (event.type === "message_reply") {
      ...
      const candidates = allUsers.filter(u =>
        ...
        (
          ...
          (senderGender !== "MALE" && senderGender !== "FEMALE")
        )
      );
    ...
    const matchRates = ["0", "-1", "99.99", "-99", "-100", "101", "0.01"];
    const matchPercent = Math.random() < 0.9
      ? Math.floor(Math.random() * 100).toString()
      : matchRates[Math.floor(Math.random() * matchRates.length)];

    // Send result
    return api.sendMessage({
      body: `『💗』Congratulations ${name1}『💗』\n` +
            `『❤️』Looks like you're destined to be with ${name2}『❤️』\n` +
            `『🔗』Your link percentage: ${matchPercent}%『🔗』`,
      mentions: [
