...
module.exports = {
  config: {
    ...
    role: 0,
    shortDescription: {
      en: "Find your perfect pair (ver 2)"
    },
    longDescription: {
      en: "Pairs you with someone in the group using a stylish background"
    },
    category: "love",
    guide: {
  ...
  onStart: async function ({ api, event, usersData }) {
    const pathImg = __dirname + "/assets/pair2_bg.png";
    const pathAvt1 = __dirname + "/assets/pair2_avt1.png";
    const pathAvt2 = __dirname + "/assets/pair2_avt2.png";
    const backgroundUrl = "https://raw.githubusercontent.com/alkama844/res/refs/heads/main/image/pair2.jpg";
    ...
    const specialRates = ["0.01", "101", "99.99", "∞", "-100"];
    const rate = Math.random() < 0.85
      ? Math.floor(Math.random() * 100)
      : specialRates[Math.floor(Math.random() * specialRates.length)];

    const message = `✨ Pairing Complete ✨\n\n` +
      `💞 ${name1} + ${name2}\n` +
      `❤️ Link Compatibility: ${rate}%\n\n` +
      `Destiny has spoken. May your hearts be synced.`;
